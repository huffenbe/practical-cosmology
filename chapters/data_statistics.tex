...
\end{equation}
The standard deviation\index{standard deviation} is simply the square root of the variance.  As we see in the exercises, another expression for the variance is
\begin{equation}
  \sigma^2 = \left\langle x^2\right\rangle - \mu^2. \label{eqn:variance_diff_squares}
\end{equation}
A Gaussian distribution is totally described by the mean and variance, and the higher moments can be expressed in terms of them.  The variance for the Poisson distribution has the same value as its mean $\mu$.  General non-Gaussian probability distributions may have information beyond what is in the mean and variance.   The third central moment measures the skewness, how asymmetric the distribution is around the mean.  The fourth central moment, kurtosis, measures how much of a peak or plateau the distrubution has.\footnote{Formal definitions of skewness and kurtosis are normalized by the standard deviation.}  
...
Suppose that we have fortunately reproduced the true signal with our model and our probability is Gaussian.  Then the $\chi^2$ statistic is simply the sum of $n_i^2/\sigma_i^2$, or the sum of a $k$ number of unit-variance Gaussian random variables.
\begin{equation}
  \chi^2 = \sum_{i=1}^k Z_i.  \label{eqn:sum_of_gaussians}
\end{equation}
In that case, $\chi^2$ as a random variable is drawn from the $\chi^2$-distribution with $k$ degrees of freedom.  The probability distribution function is 
\begin{equation}
...
\section{Generating random deviates}
In later chapters, as we measure the cosmological parameters, we will rely on sampling methods to draw examples of cosmological models or to simulate random data sets.  Fortunately, scientific and numerical computing libraries provide the ability to generate random variables from many different distribution functions (\texttt{scipy.stats} offers well more than 100 options).

Sometimes, however, we need to sample from a probability distribution that is not in that standard set.
We will discuss two elementary ways to do that here, and then discuss several more serious methods in Chapter \ref{ch:cosmological_parameters}.
...
\item From the definition of variance, derive the following relation.
  \begin{equation}
...
\item For the data set in Exercise \ref{ex:chi2_data}, what is the upper limit on $A$?  Use the probability survival function for $\chi^2$ to determine the value  that we are confident with 95 percent probability that $A$ is below, $P(\chi^2 > \chi^2(A)| k) = 0.05$ or equivalently, $$ \int_0^{\chi^2(A)} dx\,p(x = \chi^2|k) = 0.95.$$

\item Write two routines to generate random variables values from a cosine-shaped distribution on the interval $[-\pi/2,\pi/2]$.
  \begin{enumerate}
  \item Use rejection sampling.
  \item Use inversion sampling.
  \end{enumerate}
Verify each routine by making histograms of the random deviate values.  Which algorithm is faster for large numbers of deviates?
