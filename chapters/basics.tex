...
\end{equation}
We recognize that this is the proper integral to use whether or not we are actually trying to evaluate it.  With the metric, we can compute the distance between any start- and end-point along any path.  The metric acts like a set of straps connecting points in the space and giving it structure.

We also could change the coordinates that we use to define the space and trace out any given path, and the metric adjusts so that we'll still get the same distances between physical points.  For example, a Euclidean space in spherical polar coordinates has a metric
\begin{equation}
...
We don't even need to get relativity to get experience with coordinates in curved spaces.  We can describe in polar coordinates the 2-d surface of the sphere (Figure~\ref{fig:2d_spherical_surface}), a surface defined in cartesian coordinates by the equation $x^2 + y^2 + z^2 = R^2$.  Then $R$ is the radius of the sphere, and we let $r$ be the cylindrical distance from polar axis to a point on the sphere and $\phi$ be the azimuthal angle of that point to the $x$-axis.  It is not too much trouble to show that the length of a path along the surface from the north pole to a point cylindrical coordinate $r$ is 
\begin{equation}
  \ell_{\rm radial} = \int_0^r \frac{dr'}{\sqrt{1-r'^2/R^2}} = R \sin^{-1}(r/R) > r. \label{eqn:2d_sphere_path_radius}
\end{equation}
So labeling points in the 2-d space (i.e.\ the surface) using $r$ as a coordinate, we find that while it does successfully labels the position away from the origin, it is not the (larger ) physical distance $\ell_{\rm radial}$ from the origin on a direct path through the surface.
...
\end{equation}
where $r$ is a radial coordinate (called a comoving coordinate) but again is slightly different to what we are used to, and $\theta$ and $\phi$ are angular coordinates that work normally.  This metric is extremely important.  The parameter $k$ denotes the three different kinds of spacetimes that are allowed by our restrictive conditions.  Values $k = \{ -1, 0, +1 \}$ mean that the space has negative curvature, is spatially flat, or has positive curvature.    All of this is familiar from our discussion of curved spaces above. We can call our position the origin of our coordinate system, $r=0$. 
Cosmology is concerned with the very largest size scales in the universe and we are essentially stuck in one spot, in the Milky Way. 
...
We call this metric the Friedmann-Robertson-Walker (FRW) or Friedmann-Lemaitre-Robertson-Walker (FLRW) metric after the people who found and worked with it first.  We have measured the spatial curvature and know that the radius of curvature exceeds $R_0 > 60$\ Gpc (at 95\% confidence in Planck satellite data), so our universe is fairly flat.  It might be much flatter than that.  In some instances, we will restrict to that the case.
...
\end{equation}
All determinations of distance depend on the Hubble constant; it is an enormously important quantity and there is still some few percent disagreement about its value.  The inverse of the Hubble constant can be expressed as a time, about 14 Gyrs, or a distance, about 4.3 Gpc.  Sometimes the Hubble constant is compared to reference values to make a new parameters such as
\begin{equation}
  h = h_{100} = H_0 /(\mbox{100 km/s/Mpc}) \quad\mbox{or}\quad h_{70} = H_0 /(\mbox{70 km/s/Mpc}).%,\mbox{ etc.} 
\end{equation}
You will often see these in the literature.

In flat space ($k=0$), the physical distance (like you would measure with a ruler) from the origin to a point depends on the time and the comoving coordinate as
\begin{equation}   
  \ell_{\rm radial} = a(t) r  \qquad\mbox{(flat case)}
\end{equation}
while the physical velocity of an object is
\begin{equation}
  v_{\rm phys} = \frac{d(\ell_{\rm phys})}{dt} = \frac{da}{dt} r + a \frac{dr}{dt} = \frac{\dot a}{a} a r + a \frac{dr}{dt} = H \ell_{\rm phys} + v_{\rm pec}   \quad\mbox{(flat case)} \label{eqn:vpec_flat}
\end{equation}
...
Much of the development of the universe clearly depends on the \textit{expansion history}\index{expansion history}, the scale factor as a function of time: $a(t)$.  Treating the contents of the universe the as a uniform fluid, the Einstein equations of general relativity---which relate the spacetime expressed by the metric (and its derivatives) to the matter-energy content---yield two differential equations that relate the scale factor to the energy density $\rho$ and pressure $P$ of the fluid. These equations are the first Friedmann equation,
\begin{equation}
...
\end{equation}
is constant.  The density of this component does not dilute with expansion of the universe.  This is very weird and  does not fit with our intuition of how substances work.  As we will see, this negative pressure causes it to have gravity that pushes rather than pulls.  Also very weird.  However, current modeling requires this component, dubbed ``dark energy,'' to account for about 70 percent of our present-day energy budget.
...
\begin{equation}
  ...
  \right. 
\end{equation}
Taking the time derivative, we can compute the Hubble parameter in each case as well.
\begin{equation}
  H = \frac{\dot a}{a} = \left\{
  \begin{array}{lll}
    \frac{2}{3}(1+w) t^{-1} =  \left\{ \begin{array}{l} (2/3) t^{-1} \propto a^{-3/2} \\ t^{-1} \propto a^{-2} \end{array} \right. & \begin{array}{l} w=0  \\ w = 1/3 \end{array} &  \begin{array}{l} \mbox{matter-dom.}  \\ \mbox{radiation-dom.} \end{array} \\
    \mbox{constant} & \begin{array}{l}w = -1 \end{array}& \begin{array}{l} \Lambda\mbox{-dom.} \end{array}
  \end{array}
  \right. 
\end{equation}

There are several things to note about these solutions.  The matter and radiation solutions are increasing in size but decelerating (concave-down curves), and we understand this in terms of the attractive gravitational pull of all the matter in the universe slowing the expansion.  In a flat universe, however, it is not enough to arrest the expansion in finite time.


In a flat, cosmological-constant-dominated Universe, by contrast, the expansion is exponentially accelerating.  The Hubble parameter is always constant in this case.   We think that our universe today is in the process of transitioning from being matter dominated to dark-energy dominated, as the expansion dilutes the matter but not a cosmological constant.  The components had equal energy density about $z=0.3$.  The matter-dominated phase lasted about 10 Gyr.  Because of the density scaling with scale factor, prior to the matter dominated phase was a radiation dominated phase (equal densities about $z=3400$).  The radiation dominated phase lasted only about 50 kyr.  Looking backward in time, both matter- and radiation-dominated universes reach zero size a finite time in the past.  This is the origin of the idea of the ``hot Big Bang.''  We additionally think there was another, earlier epoch of exponential expansion prior to the radiation dominated phase.  This is \textit{inflation}\index{inflation} and we understand that even less than dark energy.  We'll talk about it more when we discuss the cosmic microwave background.

Because our Universe has multiple components, and curvature is always a possibility, we need to treat the solution for $a(t)$ more generally.  From the first Friedmann equation (\ref{eqn:friedmann1}), we see that for a particular expansion rate ($H_0$) there is a particular \textit{critical density}\index{critical density} associated with flatness.  To have a flat ($k=0$) universe today ($a=1$, $H=H_0$), we must have
%\begin{equation}
...
\begin{equation}
  \Omega_k = \frac{-k/R_0^2}{H_0^2}   \qquad \mbox{or} \qquad R_0 = \frac{1}{H_0\sqrt{-\Omega_k / k}}  = \frac{1}{H_0\sqrt{|\Omega_k|}},
\end{equation}
...
\end{equation}
where we have defined a function $E(a)$ to separate the Hubble constant dependence from the density dependence. Note that for $a=1$, this form shows that the sum of the $\Omega$ parameters is unity,
\begin{equation}
  \Omega_r + \Omega_m  + \Omega_k  +  \Omega_\Lambda  = 1, 
\end{equation}
and while $\Omega_k$ (and potentially $\Omega_\Lambda$ but not in actuality) could be negative, the others can only be positive.  Rearranging, we get a separable equation
\begin{equation}
...
\end{equation}
Many of the two-component universes have analytic solutions, but our $\Lambda$CDM universe needs to be numerically integrated and it is trivial to do so to whatever accuracy we require.  Note that $1/H_0$ is the normalization for any timescale.
...
The redshift of objects is an observable that is relatively easy to measure from spectral lines, directly provides the amount of expansion in the universe since that time, and gives us one notion of the distance to an object.  We need to understand the relationship between redshift and the other observables that are also related to distance, most prominently the brightness and angular size of objects.  Brightness and angular size are also relatively easy to measure on the sky.  Comparisons of these observables and their associated distance--redshift relationships over cosmic time give us useful ways to constrain the cosmological model. 
 
Important in the discussion of distances are the concepts of \textit{standard candles}\index{standard candle} and \textit{standard rulers}\index{standard ruler}.  A standard candle is an object of known luminosity that we can compare to its measured brightness to determine the distance.  The most well-known standard candles are type Ia supernovae (SNe Ia), exploding white dwarf stars.  These are sometimes referred to as ``standardizable candles'' since they are not all identical, and their luminosity is not known from first principles, but the luminosity can be well calibrated by a series of observations that we will describe in Chapters \ref{ch:standard_candles} and \ref{ch:distance_ladder}.  A related concept is a standard siren\index{standard siren}, which is a gravitational wave source of known power that we can compare to the locally measured strain.  An example would be a binary black hole merger that is (1) understood in such detail that the power is known and is (2) localized such that the redshift of the host galaxy is measureable.
...
\begin{equation}
  ...
  \right.
\end{equation}
You can compare Equation~(\ref{eqn:2d_sphere_path_radius}) to see that $\chi$ is the radial distance in the (possibly) curved space.  We do this all at the present day, when $a=1$.  We call $\chi$ the comoving distance\index{comoving distance}: the physical distance today to the comoving coordinate $r$.

Writing the FRW metric using the comoving distance $\chi$, rather than $r$, as the radial coordinate, we have alternate forms
\begin{eqnarray}
  ds^2 &=& -dt^2 + a^2 \left[ d\chi^2 + S_k^2(\chi) d\Omega^2 \right] \\ \nonumber
  &=& a^2 \left[ -d\eta^2 +  d\chi^2 + S_k^2(\chi) d\Omega^2 \right],   
\end{eqnarray}
where $\eta$ is called the conformal time\index{conformal time}, marking a clock that slows down with the expansion of the universe.  Photons travel on null geodesics, paths with $ds^2 = 0$, so a photon on a radial path ($d\theta = d\phi = 0$) will have
\begin{equation}
  d\chi = \frac{dt}{a(t)} = - \frac{dz}{H(z)}
\end{equation}
where we have used the differential of $1+z = 1/a(t)$ in the last equality.  Integrating, we find
\begin{equation}
  \chi = \int_{\eta_\textrm{em}}^{\eta_\textrm{now}} d\eta = \int_{t_{\rm em}}^{t_{\rm now}} \frac{dt}{a(t)} = \int_0^z \frac{dz'}{H(z')}  = \frac{1}{H_0} \int_0^z \frac{dz'}{E(z')} \label{eqn:chi_of_z}
\end{equation}
to get $\chi(t)$ or more usefully $\chi(z)$. As we see, this distance--redshift relation measures the expansion history via the integrated $1/a(t)$ or $1/H(z)$.

A word about $H_0$ dependence is relevant here.  We see that $\chi(z)$ is $H_0^{-1}$ times a function of $z$ and $\Omega$ parameters.  In not-flat universes, the curvature scale $R_0 = H_0^{-1}|\Omega_k|^{-1/2}$ is also  $H_0^{-1}$ times a function of $\Omega$ parameters.  Thus the $H_0$ cancels in the $\chi/R_0$ argument of the $\sinh$ and $\sin$ functions in $S_k(\chi)$; the argument depends on $\Omega$ parameters alone.  Therefore we see that in all three cases, $r = S_k(\chi)$ is a function of $H_0^{-1}$ times a function of $\Omega$ parameters.  This decoupling allows, for example, supernova measurements to determine $\Omega$ parameters (i.e.\ the composition of the Universe) even if they do not determine $H_0$.
...
\begin{figure}[!t]
  ...
  \end{center}
  \caption{Diagrams that help explain cosmological distance measures.  The transverse comoving distance is based on an arc drawn at the present day, so the expansion of the universe doesn't affect it. In a flat geometry, $D_M = \chi$ and this is exactly the familiar relation.   The angular diameter distance helps us account for the size that a physical length (in the past at redshift $z$) will expand into.  The line-of-sight comoving Hubble distance accounts for the expansion of the universe as light travels from the far side to the near side of an object.
}
  \label{fig:distances1}
\end{figure}

\paragraph{Comoving distance.}
As mentioned, we call $\chi$ the comoving distance or the comoving radial distance.  It is the physical distance today if we were to measure with a ruler to the point where the emission (previously) came from.  We also see that $\chi = \eta_\textrm{now} - \eta_\textrm{em}$ keeps track of the conformal time.

\paragraph{Line-of-sight proper distance.}  If, the comoving distance $\chi(z)$ tells us the distance \textit{today} to a point that emitted light at redshift $z$, the proper distance is the distance \textit{at the time of emission}:
\begin{equation}
  d_p(z) = a(z) \chi(z).
\end{equation}


\paragraph{Transverse comoving distance.}  This also sometimes called the comoving angular distance or the metric distance and people often use the symbol $D_M$.  This is the distance used to compute an arc length \textit{as measured today}, so we are not worried about the expansion of the universe.  So an arc with angle $\delta\theta$ and radius $\chi$ has an arc length $D_M \delta\theta$.  See Figure~\ref{fig:distances1}.  We have already encountered this quantity, which is
\begin{equation}
  D_M(z) = S_k(\chi(z)) = r(z),
\end{equation}
finding that it is the same value as the comoving coordinate because the angular part of the metric looks the same as in standard spherical-polar coordinates.  In a flat universe only, this quantity is also the comoving distance $\chi$.
...
\begin{equation}
   \delta z = \frac{\delta\ell/a}{D_H} = \frac{\delta\chi}{D_H}.
\end{equation}
This ratio of comoving lengths also returns when we talk about fixed-comoving-sized baryon acoustic oscillations in galaxy power spectra, and their line of sight component.
Here we're using the symbol $D_H(z)$ as in the BAO literature \citep[e.g.][]{2025JCAP...02..021A}, but be wary of a variety of usages out there.\footnote{Different from us, \citet{1999astro.ph..5116H} uses a constant $D_H = c/H_0$ which agrees with us only at redshift $z=0$ in units where $c=1$.  The documentation for the Core Cosmology Library \citep{2019ApJS..242....2C} uses a different definition $D_H = cz/H_0$ in their function \texttt{pyccl.background.hubble\_distance()}.  Be sure you know what your symbols mean!}
  ...
  \paragraph{Angle-averaged comoving volume distance}  The comoving volume of a element that subtends a solid angle $\delta\Omega$ and radial depth $\delta z$ is just the comoving radial distance interval times the comoving surface area:
\begin{equation}
  \delta V = \delta\chi \, D_M^2 \delta\Omega = D_H \delta z \, D_M^2 \delta\Omega.
\end{equation}
The resultant comoving volume element\index{comoving volume element} $dV = D_H D_M^2 dz d\Omega$ is crucial when making theory computations about the universe.

The BAO community makes a related distance measure, the angle-averaged comoving volume distance\index{angle-averaged comoving volume distance},
\begin{equation}
  D_V = (z D_H D_M^2)^{1/3}, 
\end{equation}
where the $z$ factor is inserted to ensure that it matches the other distance measures at low redshift.
...
The recession velocity of an object is
\begin{equation}
  \dot d_p = H a \chi + a \dot \chi = H d_p + v_{\textrm{pec},\parallel}
\end{equation}
generalizing the Hubble flow and peculiar velocity term of Equation \ref{eqn:vpec_flat} to possibly non-flat spaces.

In any case, the total magnitude squared of the peculiar velocity is
\begin{equation}
  v_\textrm{pec}^2 = a^2 \left[\dot \chi^2 + S_k(\chi)^2 (\dot \theta^2 + \sin^2\theta \dot \phi^2)\right].
\end{equation}
where we see the angular portion of the velocity acts the same as in standard spherical polar coordinates, with $r = S_k(\chi) = D_M$.

\textcolor{purple}{Note that $D_M = S$ is the distance for transverse proper motions.  Redshift and peculiar redshift.  ``Superluminal'' quasar jets?}
...
\item Solve the Friedmann equation to get $a(t)$ for flat universes that are
  \begin{enumerate}
...
\item Show that a flat, $\Lambda$-dominated universe always has a constant Hubble parameter.

\item Show that
  \begin{equation} \frac{dt}{a(t)} = - \frac{dz}{H(z)}. \end{equation}


\item Write a computer program or make a spreadsheet that computes the comoving distance $\chi$ to redshift $z=2$ for a flat $\Lambda$CDM model with $H_0 = 70$ km/s/Mpc, $\Omega_m = 0.3$, and $\Omega_\Lambda = 0.7$.  Compute the needed integral (Equation \ref{eqn:chi_of_z}) as a Riemann sum.
