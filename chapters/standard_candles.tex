...
In SNIa cosmology,  the supernova luminosities are tradiationally characterized using a parameter for the B-band maximum light of an idealized, fiducial supernova.  In the end, the particular band doesn't matter so much: the distance modulus is the thing that we seek.
...
\begin{enumerate}
\item For a top-hat band function $R(\lambda)$ between 500--700 nm, what is $K$-correction at redshift $z=0.5$ for a source with (a) a constant spectrum with wavelength; and (b) a red spectrum $l_\lambda \propto \lambda^{0.5}$.
