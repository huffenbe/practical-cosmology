...
As few of these systems are distant enough to be in the Hubble flow, and their precisions make them really excellent distance indicators, but they are rare and the measurements difficult.
...
The evolution of the phase and frequency depends on the masses and spins of the members and the chirp mass.  For non-spinning black holes, the chirp mass alone determines the derivative of the frequency and the inspiral rate and is well determined by measuring the waveform.
...
\end{equation}
In this simplest case the ratio of pixel flux variance to the mean yields the mean flux of a single star, which is inversely proportional to the luminosity distance squared.
...
\item \label{ex:SNII-p} Work through an example of the expanding photosphere method for a Type II-p supernova, but consider the emission as a blackbody as a simplification.
  ...
  \item Argue that the ratio of the radii of the photospheres at $t_1$ and $t_2$ is
    ...
    $$
  \item Solve for $t_* = (t_1 - C t_2)/(1-C)$ where
    $$
    C = \frac{v_2}{v_1} \left( \frac{ F_1}{ F_2} \right)^{1/2} \frac{T_2^2}{T_1^2}
    $$
  \item What then an expression for the luminosity distance $D_L$ in terms of $t_*$ and other quantities?

    This exercise is just to convince you that the measure is feasible.  An actually application wouldn't solve for the distance but perform a multiparameter optimization to the (noisy) data, where $D_L$ is one of the parameters.
  \end{enumerate}
