...
We want to use the relationship between the apparent brightness and the known (or calibrated) luminosity of objects to measure the cosmological model through the luminosity distance $D_L(z)$, ultimately depending on comoving distance $\chi$ and the expansion history $a(t)$.  There is a decoupling between the parameters: the amplitude of $\chi(z)$ depends only on $H_0^{-1}$ while the shape depends only on the the $\Omega$ parameters.

Type Ia supernovae are luminous and fairly consistent explosions of white dwarf stars and have been the workhorses for cosmological measurements of this type.  They are complicated objects, not understood from first priciples, but are well-characterized and standardized.  They are luminous enough and common enough to be seen in large numbers well into the Hubble flow.  
...
The statistical methods to fit a cosmological model to these distance measurements are discussed in Chapter \ref{ch:likelihoods_cosmology}.
...
In SNIa cosmology,  the supernova luminosities are characterized using a parameter for the B-band maximum luminosity of an idealized, fiducial supernova.  From the supernova data, researchers build a model of that fiducial supernova and of the natural variations around it.  Then on a per supernova basis, detailed modeling estimates the apparent brightness at maximum light (even if the observations missed it). The modeling also estimates the ``time-stretch'' of the light curve and the color variation away from the fiducial model.  These seem to be the dominant corrections that relate the maximum-light luminosity for each supernova to the fiducial one, and under that hypothesis, the slopes for these corrections are fit as global nuisance parameters along with the cosmology.  We'll get into the details of this process.

The comoving distance $\chi(z)$ is proportional to $H_0^{-1}$ and, in a flat model, so are $D_M(z)$ and $D_L(z)$, while the shapes of these functions are determined by the $\Omega$ parameters.  For the distance modulus $\mu(z)$ in the flat model, which is a logarithm of $D_L$, changing the amplitude $H_0^{-1}$ amounts to an constant offset with $z$, and again the shape is determined by the $\Omega$ parameters.

At a measured flux (\ref{eqn:flux_from_redshift}), the distance we infer  is also proportional to the amplitude of the luminosity.  Thus in the flat case, there is a perfect degeneracy between the $H_0$ parameter and the fiducial luminosity parameter.   Again, our universe flat or nearly so, and this degeneracy is exact or very strong.  With the supernovae alone, we are therefore unable to measuring either the Hubble constant or the absolute luminosity of the fiducial supernova, but are notheless able to measure the $\Omega$ parameters because they determine the shape of the distance--redshift relation.  The shape uses only the information on the relative distances of the supernovae with redshift.
...
The template $M_0(t,\lambda)$ is a spectral sequence as a function of time for the fiducial supernova.  The amplitude is normalized for a supernova at 1 Mpc, {which gives close to zero B-band apparent magnitude at maximum light} ($t=0$). {The corresponding $x_0$ is the overall amplitude for a particular supernova's spectrum, recording the overall brightness}.

The template $M_1(t,\lambda)$ is the dominant correction among supernovae to the time evolution and spectral shape (approximating the effect of a time stretch);  the $x_{1,i}$ parameter is the fractional contribution (relative to $x_{0,i}$) of that correction.  The $M_1$ and higher corrections change the spectral shape, and thus the $K$ correction, but are constructed not to change the B-band magnitude at maximum light.
...
\begin{eqnarray}
  \chi^2_{\rm data} = \sum_{{\rm SN}\ i} && \left[ \sum_{{\rm photo\ pt.}\ q} {\left(m_{iq} - m_{{\rm band}\ q}[f_{\lambda,i} (t_q,x_i,c_i)|\hat n_i,z_i]\right)^2}/{\sigma_{iq}^2} + \right.
    \\ \nonumber
    &&  \ \left. \sum_{{\rm spec.\ pt.}\ q} (f^{\rm obs}_{\lambda_q}(t_q) - a_{\rm recal}(\lambda_q) f_{\lambda_q}(t_q,x_i,c_i|\hat n_i,z_i) )^2/\sigma_{{\rm spec},i}^2    \right].
\end{eqnarray}
...
\begin{equation}
  \chi^2_{\rm reg} = A \sum_k M_k^\dag D_2^\dag D_2 M_k, 
\end{equation}
where the matrix $D_2$ takes the derivatives, and $A$ is a tunable parameter to control its strength.
...
The  standard estimator for the distance modulus \citep{1998A&A...331..815T} is below, although I have separated what is usually combined into Equations (\ref{eqn:SNIa_tripp_distance_estimator1}) and (\ref{eqn:SNIa_tripp_distance_estimator}).  For each supernova $i$, the distance modulus is written
\begin{eqnarray}
...
The absolute magnitude for each supernova is measured as an overall normalization for the luminosity of the fiducial supernova ($x_1 = 0$, $c = 0$), and empirical corrections for shape and color,
\begin{equation}
  M_{B,i} =  {\cal M}_B - \alpha x_{1,i} + \beta c_i  \label{eqn:SNIa_tripp_distance_estimator2}
\end{equation}
where ${\cal M}_B$ is a pseudo-absolute magnitude that makes the degeneracy explicit between the zero-point for the SN calibration and the Hubble constant\footnote{This equation sometimes appears with a constant offset of 25 magnitudes, accounting for the difference between 10 pc and 1 Mpc distance normalization.}:
\begin{equation} {\cal M}_B= M_B - 5 \log_{10}\left(\frac{H_0}{100 \mbox{ km/s/Mpc}} \right) \end{equation}  
The B-band absolute magnitude $M_B$ parameterizes the fiducial luminosity at maximum light we would measure if the Hubble constant were $H_0 = 100$ km/s/Mpc.  The final two terms in Equations (\ref{eqn:SNIa_tripp_distance_estimator}) and (\ref{eqn:SNIa_tripp_distance_estimator2}) record the adjustment of the maximum luminosity.  The parameters $\alpha$ and $\beta$ are the coefficients of the light curve shape and color corrections common across the whole population of supernovae.  These parameters are fit to minimize the residuals in the distance modulus versus redshift diagram.  They can be fit at the same time as the cosmology.

Sometimes an additional term, a step-function of mass, is added to the distance estimator as a third correction.  It was noted that supernovae in more massive galaxies appeared less luminous than in low-mass galaxy hosts.  The step parameters are also fitted to minimize residuals in the Hubble diagram.

As mentioned at the beginning of this section (\ref{sec:measuring_distance}), even without knowing the intrisic luminosity of Type Ia, you can get a measurement of the relative distance between redshifts. The difference of distance moduli, $\mu_i - \mu_i'$, which corresponds to a ratio of distances, causes ${\cal M}_B$ to cancel while preserving the stretch- and color-based luminosity corrections.  This contains information on cosmological parameters except for $H_0$, which exactly sets the overall normalization of the distance scale in a flat universe.  Thus supernova data sets can be used in conjunction with CMB or BAO data to yield tighter cosmological constraints, whether or not an absolute luminosity normalization is employed.  %However, using the normalized distance enhances the supernova constraints significantly.

To measure $H_0$ as well, in the framework of Equation (\ref{eqn:SNIa_tripp_distance_estimator}), the overall normalization of the luminosity is set by the single parameter, $M_B$.  For each SALT2-fitted supernova with a \textit{known} distance, determined by the Cepheid variable star period-luminosity relation (or other means), $M_B$ becomes the only unknown.  A weighted average over these supernova yields the value $M_B$, which determines the absolute distance of everything else.  We explore the distance ladder in the next chapter, which makes measurement of $H_0$ possible.
